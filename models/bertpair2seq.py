...
from module.constrained_beam_search_decoder import ConstrainedBeamSearchDecoder
from module.layers import term_id_extract, term_emb_extract, convert2vec
from module.bert import BertModel
from utils.fix_param import DictParam
#dict config has high priority
class BertPair2SeqConfig(BaseModelConfig):
    def __init__(self,config_dict,config_name='bertpair2seq'):
        super(BertPair2SeqConfig,self).__init__(config_name)
        #from json
...
class BertPair2Seq(BaseModel):
    def __init__(self, config_dict):
        super(BertPair2Seq,self).__init__()
        self.cfg = BertPair2SeqConfig(config_dict)
    ...
    def inference(self, input_fields, mode):
        if mode == tf.contrib.learn.ModeKeys.TRAIN or mode == tf.contrib.learn.ModeKeys.EVAL:
            query, doc = input_fields["query:bertpair:0:0"], input_fields["doc"]
        else:
            query,doc = input_fields["query:bertpair:0:0"], None
    ...
    def encoder(self, query, mode):
        query, qtype = query
        token_type_ids = convert2vec(qtype)
        q_ids,q_mask = term_id_extract(query, self.bert_dict)
        q_len = tf.count_nonzero(q_mask,axis=-1)
        QBert = BertModel(self.cfg, mode == tf.contrib.learn.ModeKeys.TRAIN, input_ids=q_ids, input_mask=q_mask, token_type_ids=token_type_ids,use_one_hot_embeddings=False,scope="bert")
    ...
    def decoder(self, encoder_outputs, encoder_state, encoder_sequence_length, doc, mode):
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE) as decoder_scope:
            ...
            decoder_cell = self.build_cell('decoder_0', self.cfg.hidden_size)
            if not self.cfg.dim_attention == self.cfg.hidden_size:
