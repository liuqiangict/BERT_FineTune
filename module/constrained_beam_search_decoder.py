...
class ConstrainedBeamSearchDecoderState(
    collections.namedtuple("ConstrainedBeamSearchDecoderState",
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
  ...
  next_beam_ids = math_ops.to_int32(
      math_ops.div(word_indices, vocab_size), name="next_beam_parent_ids")
  ...
  next_prediction_len = _tensor_gather_helper(
      ...
      range_size=beam_width,
      gather_shape=[-1])
  next_prediction_len = lengths_to_add + next_prediction_len
...
def _get_scores(log_probs, sequence_lengths, length_penalty_weight,
                coverage_penalty_weight, finished, accumulated_attention_probs, constrained_matrix):
  ...
  if tensor_util.constant_value(coverage_penalty_weight) == 0.0:
    return scores if constrained_matrix == None else scores + constrained_matrix
  ...
  return scores + weighted_coverage_penalty if constrained_matrix == None else scores + weighted_coverage_penalty + constrained_matrix
...
def _mask_probs(probs, eos_token, finished):
  ...
  finished_row = array_ops.one_hot(
      ...
      #off_value=ops.convert_to_tensor(-np.inf, dtype=probs.dtype))
