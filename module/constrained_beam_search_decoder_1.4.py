...
__all__ = [
    "ConstrainedBeamSearchDecoderOutput",
    "ConstrainedBeamSearchDecoderState",
    "ConstrainedBeamSearchDecoder",
    "FinalConstrainedBeamSearchDecoderOutput",
    "tile_batch",
]


class ConstrainedBeamSearchDecoderState(
    collections.namedtuple("ConstrainedBeamSearchDecoderState", ("cell_state", "log_probs",
                                                      "finished", "lengths","accumulated_attention_probs"))):
  pass


class ConstrainedBeamSearchDecoderOutput(
    collections.namedtuple("ConstrainedBeamSearchDecoderOutput",
                           ("scores", "predicted_ids", "parent_ids"))):
  pass


class FinalConstrainedBeamSearchDecoderOutput(
    collections.namedtuple("FinalConstrainedBeamDecoderOutput",
                           ["predicted_ids", "beam_search_decoder_output","scores"])):
...
class ConstrainedBeamSearchDecoder(decoder.Decoder):
  ...
  def __init__(self,
    ...
    """Initialize the ConstrainedBeamSearchDecoder.
  ...
  def output_size(self):
    ...
    return ConstrainedBeamSearchDecoderOutput(
  ...
  def initialize(self, name=None):
    ...
    #Amanda: Copy coverage penalty
    dtype = nest.flatten(self._initial_cell_state)[0].dtype
    log_probs = tf.one_hot(tf.zeros([self._batch_size], dtype = dtypes.int32),
    ...
    initial_state = ConstrainedBeamSearchDecoderState(
  ...
  def finalize(self, outputs, final_state, sequence_lengths):
    ...
    outputs = FinalConstrainedBeamSearchDecoderOutput(
        beam_search_decoder_output=outputs, predicted_ids=predicted_ids,scores=outputs.scores)
  ...
  def step(self, time, inputs, state, name=None):
    ...
    if not constrained_matrix == None:
        ...
        constrained_matrix = tf.tile(constrained_matrix, [1,beam_width,1])
    with ops.name_scope(name, "ConstrainedBeamSearchDecoderStep", (time, inputs, state)):
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
                      beam_width, end_token, length_penalty_weight, coverage_penalty_weight,constrained_matrix):
  ...
  total_probs = array_ops.expand_dims(beam_state.log_probs, 2) + step_log_probs # For end beam: set to [-inf,-inf,-inf,-inf], else: set to sum of historical log prob
...
def get_attention_probs(next_cell_state, coverage_penalty_weight):
  ...
  if not probs_per_attn_layer:
    raise ValueError("coverage_penalty_weight must be 0.0 if no cell is attentional.")
...
def _get_scores(log_probs, sequence_lengths, length_penalty_weight, coverage_penalty_weight, finished, accumulated_attention_probs, constrained_matrix):
  ...
  scores = log_probs / length_penality_
  ...
  if tensor_util.constant_value(coverage_penalty_weight) == 0.0:
    if constrained_matrix == None:
        return scores
    else:
        return scores + constrained_matrix
  ...
  accumulated_attention_probs = tf.where(tf.equal(accumulated_attention_probs, 0.0), tf.ones_like(accumulated_attention_probs), accumulated_attention_probs)

  coverage_penalty = tf.reduce_sum(tf.log(tf.minimum(accumulated_attention_probs, 1.0)),2)
  #coverage_penalty = tf.reduce_sum(accumulated_attention_probs,2)
  #coverage_penalty *= tf.to_float(finished)
  #coverage_penalty = tf.to_float(finished) * coverage_penalty
  ...
  if constrained_matrix == None:
      ...
      return scores + weighted_coverage_penalty + constrained_matrix
...
def _mask_probs(probs, eos_token, finished):
  ...
  #   math_ops.to_float(1. - math_ops.to_float(finished)), 2) #1 for not finished, 0 for finished
  ## These examples are not finished and we leave them
  ##[batch_size,beam_width,vocab_size], set finished beam to all zero, unifinshed beam remain unchanged
  #non_finished_examples = finished_mask * probs
  ## All finished examples are replaced with a vector that has all
  ## probability on EOS
  ##[vocab_size], [0,-inf,-inf,-inf,....]
  ...
  #finished_examples = (1. - finished_mask) * finished_row #set eos_token to 0, other token to -inf?
  #return finished_examples + non_finished_examples
  finished_row = array_ops.one_hot(   # [0,-inf,-inf,-inf,...]
      eos_token,
      vocab_size,
      dtype=probs.dtype,
      #on_value=ops.convert_to_tensor(0., dtype=probs.dtype),
      on_value=probs.dtype.min,
      off_value=probs.dtype.min)
  finished_probs = array_ops.tile( #[batch_size,beam_width, vocab_size] of ...[0,-inf,-inf,-inf,....]
  ...
  return array_ops.where(finished_mask, finished_probs, probs) # For finished beam, set the prob to [0,-inf,-inf,-inf,...], else: remain unchanged
